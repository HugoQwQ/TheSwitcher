...
public class Auth {
	...
	static {
		...
		try {
			if (BLIND_SSL) {
				...
				Shared.LOG.warn("Blindly skipping SSL checks. (behavior: 'ias.blindSSL' property)");
				...
				managers.addAll(Arrays.stream(customTmf.getTrustManagers())
						.filter(tm -> tm instanceof X509TrustManager)
						.map(tm -> (X509TrustManager) tm).collect(Collectors.toList()));
				managers.addAll(Arrays.stream(defaultTmf.getTrustManagers())
						.filter(tm -> tm instanceof X509TrustManager)
						.map(tm -> (X509TrustManager) tm).collect(Collectors.toList()));
				TrustManager multiManager = new X509TrustManager() {
					...
					public void checkClientTrusted(X509Certificate[] chain, String authType)
							throws CertificateException {
						CertificateException wrapper = new CertificateException(
								"Unable to validate via any trust manager.");
				...
				Shared.LOG.info(
						"Using shared SSL context. (behavior: default; custom + default certificates)");
			} else {
				Shared.LOG.warn("Not editing SSL context. (behavior: 'ias.noCustomSSL' property)");
			}
		} catch (Throwable t) {
			Shared.LOG.error("Unable to init SSL context.", t);
		}
		FIXED_CONTEXT = ctx;
	}
	/**
	 * Get Microsoft Access Token and Microsoft Refresh Token from Microsoft
	 * Authentication Code.
	 *
	 * @param code
	 *            Code from user auth redirect
	 * @return Pair of Microsoft Access Token and Microsoft Refresh Token
	 * @throws Exception
	 *             If something goes wrong
	 * @see <a href=
	 *      "https://wiki.vg/Microsoft_Authentication_Scheme#Authorization_Code_-.3E_Authorization_Token">Reference</a>
	 */
	public static Map.@NotNull Entry<@NotNull String, @NotNull String> codeToToken(
			@NotNull String code) throws Exception {
		HttpsURLConnection conn = (HttpsURLConnection) new URL(
				"https://login.live.com/oauth20_token.srf").openConnection();
		...
		try (OutputStream out = conn.getOutputStream()) {
			out.write(("client_id=" + URLEncoder.encode(CLIENT_ID, "UTF-8") + "&" + "code="
					+ URLEncoder.encode(code, "UTF-8") + "&" + "grant_type=authorization_code&"
			...
			if (conn.getResponseCode() < 200 || conn.getResponseCode() > 299) {
				try (BufferedReader err = new BufferedReader(
					...
					throw new IllegalArgumentException(
							"codeToToken response: " + conn.getResponseCode() + ", data: "
									+ err.lines().collect(Collectors.joining("\n")));
				} catch (Throwable t) {
					throw new IllegalArgumentException(
							"codeToToken response: " + conn.getResponseCode(), t);
				}
			}
			try (BufferedReader in = new BufferedReader(
					new InputStreamReader(conn.getInputStream(), StandardCharsets.UTF_8))) {
				JsonObject resp = Shared.GSON.fromJson(in.lines().collect(Collectors.joining("\n")),
						JsonObject.class);
	 ...
	 *
	 * @param refreshToken
	 *            Microsoft Refresh Token from {@link #codeToToken(String)} or from
	 *            this method
	 * @return Pair of Microsoft Access Token and Microsoft Refresh Token
	 * @throws Exception
	 *             If something goes wrong
	 * @see <a href=
	 *      "https://wiki.vg/Microsoft_Authentication_Scheme#Refreshing_Tokens">Reference</a>
	 */
	public static Map.@NotNull Entry<@NotNull String, @NotNull String> refreshToken(
			@NotNull String refreshToken) throws Exception {
		HttpsURLConnection conn = (HttpsURLConnection) new URL(
				"https://login.live.com/oauth20_token.srf").openConnection();
		if (FIXED_CONTEXT != null)
			conn.setSSLSocketFactory(FIXED_CONTEXT.getSocketFactory());
		...
		conn.setDoOutput(true);
		try (OutputStream out = conn.getOutputStream()) {
			out.write(("client_id=" + URLEncoder.encode(CLIENT_ID, "UTF-8") + "&" + "refresh_token="
			...
			if (conn.getResponseCode() < 200 || conn.getResponseCode() > 299) {
				try (BufferedReader err = new BufferedReader(
						new InputStreamReader(conn.getErrorStream(), StandardCharsets.UTF_8))) {
					throw new IllegalArgumentException(
							"refreshToken response: " + conn.getResponseCode() + ", data: "
									+ err.lines().collect(Collectors.joining("\n")));
				} catch (Throwable t) {
					throw new IllegalArgumentException(
							"refreshToken response: " + conn.getResponseCode(), t);
				}
			}
			try (BufferedReader in = new BufferedReader(
					new InputStreamReader(conn.getInputStream(), StandardCharsets.UTF_8))) {
	 ...
	 *
	 * @param authToken
	 *            Microsoft Access Token from {@link #codeToToken(String)} or
	 *            {@link #refreshToken(String)}
	 * @return XBL Token
	 * @throws Exception
	 *             If something goes wrong
	 * @see <a href=
	 *      "https://wiki.vg/Microsoft_Authentication_Scheme#Authenticate_with_XBL">Reference</a>
	 */
	public static @NotNull String authXBL(@NotNull String authToken) throws Exception {
		HttpsURLConnection conn = (HttpsURLConnection) new URL(
				"https://user.auth.xboxlive.com/user/authenticate").openConnection();
		if (FIXED_CONTEXT != null)
			conn.setSSLSocketFactory(FIXED_CONTEXT.getSocketFactory());
			...
			if (conn.getResponseCode() < 200 || conn.getResponseCode() > 299) {
				try (BufferedReader err = new BufferedReader(
					...
					throw new IllegalArgumentException("authXBL response: " + conn.getResponseCode()
							+ ", data: " + err.lines().collect(Collectors.joining("\n")));
				} catch (Throwable t) {
					throw new IllegalArgumentException(
							"authXBL response: " + conn.getResponseCode(), t);
				}
			}
			try (BufferedReader in = new BufferedReader(
					new InputStreamReader(conn.getInputStream(), StandardCharsets.UTF_8))) {
	 ...
	 *
	 * @param xblToken
	 *            XBL Token from {@link #authXBL(String)}
	 * @return Pair of XSTS Token and XUI-UHS Userhash
	 * @throws Exception
	 *             If something goes wrong
	 * @see <a href=
	 *      "https://wiki.vg/Microsoft_Authentication_Scheme#Authenticate_with_XSTS">Reference</a>
	 */
	public static Map.@NotNull Entry<@NotNull String, @NotNull String> authXSTS(
			@NotNull String xblToken) throws Exception {
		HttpsURLConnection conn = (HttpsURLConnection) new URL(
				"https://xsts.auth.xboxlive.com/xsts/authorize").openConnection();
		if (FIXED_CONTEXT != null)
			conn.setSSLSocketFactory(FIXED_CONTEXT.getSocketFactory());
		conn.setRequestProperty("Content-Type", "application/json");
		conn.setRequestProperty("Accept", "application/json");
		...
		try (OutputStream out = conn.getOutputStream()) {
			...
			if (conn.getResponseCode() < 200 || conn.getResponseCode() > 299) {
				try (BufferedReader err = new BufferedReader(
						new InputStreamReader(conn.getErrorStream(), StandardCharsets.UTF_8))) {
					throw new IllegalArgumentException(
							"authXSTS response: " + conn.getResponseCode() + ", data: "
									+ err.lines().collect(Collectors.joining("\n")));
				} catch (Throwable t) {
					throw new IllegalArgumentException(
							"authXSTS response: " + conn.getResponseCode(), t);
				}
			}
			try (BufferedReader in = new BufferedReader(
					new InputStreamReader(conn.getInputStream(), StandardCharsets.UTF_8))) {
				JsonObject resp = Shared.GSON.fromJson(in.lines().collect(Collectors.joining("\n")),
						JsonObject.class);
				return new AbstractMap.SimpleImmutableEntry<>(resp.get("Token").getAsString(),
						resp.getAsJsonObject("DisplayClaims").getAsJsonArray("xui").get(0)
	 ...
	 *
	 * @param userHash
	 *            XUI-UHS Userhash from {@link #authXSTS(String)}
	 * @param xstsToken
	 *            XSTS Token from {@link #authXSTS(String)}
	 * @return Minecraft Access Token
	 * @throws Exception
	 *             If something goes wrong
	 * @see <a href=
	 *      "https://wiki.vg/Microsoft_Authentication_Scheme#Authenticate_with_Minecraft">Reference</a>
	 */
	public static @NotNull String authMinecraft(@NotNull String userHash, @NotNull String xstsToken)
			throws Exception {
		HttpsURLConnection conn = (HttpsURLConnection) new URL(
		...
		try (OutputStream out = conn.getOutputStream()) {
			...
			if (conn.getResponseCode() < 200 || conn.getResponseCode() > 299) {
				try (BufferedReader err = new BufferedReader(
						new InputStreamReader(conn.getErrorStream(), StandardCharsets.UTF_8))) {
					throw new IllegalArgumentException(
							"authMinecraft response: " + conn.getResponseCode() + ", data: "
									+ err.lines().collect(Collectors.joining("\n")));
				} catch (Throwable t) {
					throw new IllegalArgumentException(
							"authMinecraft response: " + conn.getResponseCode(), t);
				}
			}
			try (BufferedReader in = new BufferedReader(
					new InputStreamReader(conn.getInputStream(), StandardCharsets.UTF_8))) {
	 ...
	 *
	 * @param accessToken
	 *            Minecraft Access Token from {@link #authMinecraft(String, String)}
	 * @return Pair of Player UUID and Player Name
	 * @throws Exception
	 *             If something goes wrong
	 * @see <a href=
	 *      "https://wiki.vg/Microsoft_Authentication_Scheme#Get_the_profile">Reference</a>
	 */
	public static Map.@NotNull Entry<@NotNull UUID, @NotNull String> getProfile(
		...
		try (BufferedReader in = new BufferedReader(
			...
			JsonObject resp = Shared.GSON.fromJson(in.lines().collect(Collectors.joining("\n")),
					JsonObject.class);
			return new AbstractMap.SimpleImmutableEntry<>(
					UUID.fromString(resp.get("id").getAsString().replaceFirst(
	...
	public static @NotNull UUID resolveUUID(@NotNull String name) {
		try (InputStreamReader in = new InputStreamReader(
			...
			UUID uuid = UUID.fromString(
					Shared.GSON.fromJson(in, JsonObject.class).get("id").getAsString().replaceFirst(
							"(\\w{8})(\\w{4})(\\w{4})(\\w{4})(\\w{12})", "$1-$2-$3-$4-$5"));
			return uuid;
