...
public class MicrosoftAuthCallback implements Closeable {
	public static final String MICROSOFT_AUTH_URL = "https://login.live.com/oauth20_authorize.srf"
			+ "?client_id=54fd49e4-2103-4044-9603-2b028c814ec3" + "&response_type=code"
			+ "&scope=XboxLive.signin%20XboxLive.offline_access"
			+ "&redirect_uri=http://localhost:59125" + "&prompt=select_account";
	private HttpServer server;
	public @NotNull CompletableFuture<@Nullable MicrosoftAccount> start(
		...
		try {
			...
			server.createContext("/", ex -> {
				Shared.LOG.info(
						"Microsoft authentication callback request: " + ex.getRemoteAddress());
				try (BufferedReader in = new BufferedReader(new InputStreamReader(
						Objects.requireNonNull(
								MicrosoftAuthCallback.class.getResourceAsStream("/authPage.html")),
						StandardCharsets.UTF_8))) {
					progressHandler.accept("ias.loginGui.microsoft.progress",
							new Object[]{"preparing"});
					...
					Shared.EXECUTOR.execute(() -> {
						try {
							...
							Shared.LOG.error("Unable to authenticate via Microsoft.", t);
							cf.completeExceptionally(t);
						}
					});
				} catch (Throwable t) {
					Shared.LOG.error(
							"Unable to process request on Microsoft authentication callback server.",
							t);
					close();
					cf.completeExceptionally(t);
				}
			...
			server.start();
			Shared.LOG.info("Started Microsoft authentication callback server.");
		} catch (Throwable t) {
			Shared.LOG.error("Unable to run the Microsoft authentication callback server.", t);
			close();
			cf.completeExceptionally(t);
		}
		return cf;
	...
	private @Nullable MicrosoftAccount auth(
			@NotNull BiConsumer<@NotNull String, @NotNull Object[]> progressHandler,
			@Nullable String query) throws Exception {
		Shared.LOG.info("Authenticating...");
		if (query == null)
		...
		Shared.LOG.info("Step: codeToToken.");
		...
		Shared.LOG.info("Step: authMinecraft.");
		progressHandler.accept("ias.loginGui.microsoft.progress", new Object[]{"authMinecraft"});
		String accessToken = Auth.authMinecraft(xstsTokenUserhash.getValue(),
				xstsTokenUserhash.getKey());
		Shared.LOG.info("Step: getProfile.");
		...
		Shared.LOG.info("Authenticated.");
		return new MicrosoftAccount(profile.getValue(), accessToken, refreshToken,
				profile.getKey());
	}
	...
	public void close() {
		try {
			if (server != null) {
				server.stop(0);
				Shared.LOG.info("Stopped Microsoft authentication callback server.");
			}
		} catch (Throwable t) {
			Shared.LOG.error("Unable to stop the Microsoft authentication callback server.", t);
		}
	}
}
