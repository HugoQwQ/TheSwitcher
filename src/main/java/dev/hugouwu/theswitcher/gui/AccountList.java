...
import dev.hugouwu.theswitcher.utils.SkinLoader;
...
public class AccountList extends GuiListExtended {
	...
	public class AccountEntry implements IGuiListEntry {
		...
		public AccountEntry(Account account) {
			...
			if (TheSwitcher.SKIN_CACHE.containsKey(account.uuid())) {
				this.skin = TheSwitcher.SKIN_CACHE.get(account.uuid());
				return;
			}
			...
			SkinLoader.loadSkin(account.uuid()).thenAccept(en -> {
				...
				mc.addScheduledTask(() -> {
					skin = mc.getTextureManager().getDynamicTextureLocation(
							"ias_skin:" + account.uuid().toString().replace("-", ""),
							new DynamicTexture(skinImage));
					TheSwitcher.SKIN_CACHE.put(account.uuid(), skin);
				});
			});
		}
		...
		public void drawEntry(int i, int x, int y, int w, int h, int mx, int my, boolean hover) {
			int color = -1;
			if (mc.getSession().getUsername().equals(account.name()))
				color = 0x00FF00;
			...
			if (mc.gameSettings.getModelParts().contains(EnumPlayerModelParts.HAT))
				GuiScreen.drawModalRectWithCustomSizedTexture(x, y + 1, 40, 8, 8, 8, 64, 64); // Head
																								// (Overlay)
			if (selectedElement == i) {
				mc.getTextureManager()
						.bindTexture(new ResourceLocation("textures/gui/server_selection.png"));
				GlStateManager.color(1F, 1F, 1F, 1F);
				boolean movableDown = i + 1 < entries.size();
				boolean movableUp = i > 0;
				if (movableDown) {
					boolean hoveredDown = mx > x + w - 16 && mx < x + w - 6 && hover;
					GuiScreen.drawModalRectWithCustomSizedTexture(x + w - 35, y - 18, 48,
							hoveredDown ? 32 : 0, 32, 32, 256, 256);
				}
				if (movableUp) {
					boolean hoveredUp = mx > x + w - (movableDown ? 28 : 16)
							&& mx < x + w - (movableDown ? 16 : 6) && hover;
					GuiScreen.drawModalRectWithCustomSizedTexture(x + w - (movableDown ? 30 : 19),
							y - 3, 96, hoveredUp ? 32 : 0, 32, 32, 256, 256);
				}
			}
		}
				...
				if (movableDown) {
					...
					if (hoveredDown) {
						mc.getSoundHandler().playSound(PositionedSoundRecord
								.create(new ResourceLocation("gui.button.press"), 1F));
						swap(i, i + 1);
					}
				}
				if (movableUp) {
					boolean hoveredUp = rx > w - (movableDown ? 28 : 16)
							&& rx < w - (movableDown ? 16 : 6);
					if (hoveredUp) {
						mc.getSoundHandler().playSound(PositionedSoundRecord
								.create(new ResourceLocation("gui.button.press"), 1F));
						swap(i, i - 1);
