...
import dev.hugouwu.theswitcher.TheSwitcher;
...
public class AccountListScreen extends GuiScreen {
	...
	public void initGui() {
		...
		buttonList.add(reloadSkins = new GuiButton(0, 2, 2, 120, 20,
				I18n.format("ias.listGui.reloadSkins")));
		search = new GuiTextField(1, this.fontRendererObj, this.width / 2 - 80, 14, 160, 16);
		buttonList.add(add = new GuiButton(2, this.width / 2 + 4 + 40, this.height - 52, 120, 20,
				I18n.format("ias.listGui.add")));
		buttonList.add(login = new GuiButton(3, this.width / 2 - 154 - 10, this.height - 52, 120,
				20, I18n.format("ias.listGui.login")));
		buttonList.add(loginOffline = new GuiButton(4, this.width / 2 - 154 - 10, this.height - 28,
				110, 20, I18n.format("ias.listGui.loginOffline")));
		buttonList.add(edit = new GuiButton(5, this.width / 2 - 40, this.height - 52, 80, 20,
				I18n.format("ias.listGui.edit")));
		buttonList.add(delete = new GuiButton(6, this.width / 2 - 50, this.height - 28, 100, 20,
				I18n.format("ias.listGui.delete")));
		buttonList.add(new GuiButton(7, this.width / 2 + 4 + 50, this.height - 28, 110, 20,
	...
	public void mouseClicked(int mx, int my, int btn) throws IOException {
		if (list.mouseClicked(mx, my, btn))
			return;
	...
	public void drawScreen(int mx, int my, float delta) {
		...
		if (search.getText().isEmpty())
			drawString(fontRendererObj, I18n.format("ias.listGui.search"), search.xPosition + 4,
					search.yPosition + (search.height - 8) / 2, 0xFF808080);
		super.drawScreen(mx, my, delta);
		drawCenteredString(fontRendererObj, "Switcher", this.width / 2, 4, -1);
		if (list.selectedElement() >= 0) {
			...
			GuiScreen.drawModalRectWithCustomSizedTexture(slim ? 1 : 0, 8, 44, 20, slim ? 3 : 4, 12,
					64, 64); // Right Arm (Left from our perspective)
			GuiScreen.drawModalRectWithCustomSizedTexture(12, 8, 36, 52, slim ? 3 : 4, 12, 64, 64); // Left
																									// Arm
																									// (Right
																									// from
																									// our
			...
			if (mc.gameSettings.getModelParts().contains(EnumPlayerModelParts.HAT))
				GuiScreen.drawModalRectWithCustomSizedTexture(4, 0, 40, 8, 8, 8, 64, 64); // Head
																							// (Overlay)
			if (mc.gameSettings.getModelParts().contains(EnumPlayerModelParts.RIGHT_SLEEVE))
				GuiScreen.drawModalRectWithCustomSizedTexture(slim ? 1 : 0, 8, 44, 36, slim ? 3 : 4,
						12, 64, 64); // Right Arm (Overlay)
			if (mc.gameSettings.getModelParts().contains(EnumPlayerModelParts.LEFT_SLEEVE))
				GuiScreen.drawModalRectWithCustomSizedTexture(12, 8, 52, 52, slim ? 3 : 4, 12, 64,
						64); // Left Arm (Overlay)
			if (mc.gameSettings.getModelParts().contains(EnumPlayerModelParts.RIGHT_PANTS_LEG))
				GuiScreen.drawModalRectWithCustomSizedTexture(4, 20, 4, 36, 4, 12, 64, 64); // Right
																							// Leg
																							// (Overlay)
			if (mc.gameSettings.getModelParts().contains(EnumPlayerModelParts.LEFT_PANTS_LEG))
				GuiScreen.drawModalRectWithCustomSizedTexture(8, 20, 4, 52, 4, 12, 64, 64); // Left
																							// Leg
																							// (Overlay)
			GlStateManager.popMatrix();
		}
		if (state != null) {
			drawCenteredString(fontRendererObj, state, this.width / 2, this.height - 62,
					0xFFFF9900);
		}
	}

	private void reloadSkins() {
		if (list.entries.isEmpty() || System.currentTimeMillis() <= nextSkinUpdate || state != null)
			return;
		TheSwitcher.SKIN_CACHE.clear();
		list.updateAccounts(search.getText());
	...
	private void login() {
		...
		acc.login((s, o) -> state = I18n.format(s, o)).whenComplete((d, t) -> {
			...
			if (t != null) {
				mc.addScheduledTask(
						() -> mc.displayGuiScreen(new AlertScreen(() -> mc.displayGuiScreen(this),
								EnumChatFormatting.RED + I18n.format("ias.error"),
								String.valueOf(t))));
				return;
			}
			mc.addScheduledTask(() -> {
				((MinecraftAccessor) Minecraft.getMinecraft()).setSession(new Session(d.name(),
						UUIDTypeAdapter.fromUUID(d.uuid()), d.accessToken(), d.userType()));
			});
		});
	}
	...
	private void loginOffline() {
		if (list.selectedElement() < 0 || state != null)
			return;
		Account acc = list.entries.get(list.selectedElement()).account();
		((MinecraftAccessor) Minecraft.getMinecraft()).setSession(new Session(acc.name(),
				UUIDTypeAdapter.fromUUID(UUID.nameUUIDFromBytes(
						"OfflinePlayer".concat(acc.name()).getBytes(StandardCharsets.UTF_8))),
				"0", "legacy"));
	}
	...
	private void updateButtons() {
		...
		reloadSkins.enabled = list.selectedElement() >= 0 && state == null
	...
	public void keyTyped(char c, int key) throws IOException {
		...
		if (key == Keyboard.KEY_DELETE || key == Keyboard.KEY_MINUS
