...
public class Expression {
	 ...
	 *
	 * @param returnFlag
	 *            Flag for return value. 0 = return at end, 1 = return at multiply,
	 *            2 = return at factor
	 * @return Parsed expression
	 * @throws IllegalArgumentException
	 *             If we can't calculate this
	...
	private double parse(int returnFlag) {
		try {
			...
			throw new IllegalArgumentException(
					"Sorry, but we're unable to calculate " + expression + " at pos " + position,
					t);
		}
	}

	/**
	 * Parse expression, replacing <code>w</code> with width, <code>h</code> with
	 * height.
	 *
	 * @param expression
	 *            Expression string
	 ...
	 */
	@Contract(pure = true)
	public static double parseWidthHeight(String expression, int width, int height)
			throws IllegalArgumentException {
		return new Expression(expression.toLowerCase(Locale.ROOT)
				.replace("w", String.valueOf(width)).replace("h", String.valueOf(height))).parse();
	}
}